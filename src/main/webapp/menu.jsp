	...
	<table class="menu">
				...
				<p class="menu">
					...
					<c:choose>
						<c:when test="${empty user.username}">
							<a href="Controller?action=sign">Sign in</a>
						</c:when>
						<c:otherwise>
							Logged in as <c:out value="${user.username}" />
							<a href="Controller?action=changepassword">Change
								password</a>
							|
							<a href="Controller?action=signOut">Sign out</a>
						</c:otherwise>
					</c:choose>
				</p>
			</th>
		...
		<c:if test="${not empty user.username}">
			<tr>
				...
				<div class="midmenu">
				<a href="Controller?action=welcome"><img
						src="buttons/button_main.png" class="midmenu" /></a> <c:choose>
						<c:when test="${user.authorization==2}">
							<a href="Controller?action=addUser"><img
								src="buttons/button_add-user.png" class="midmenu" /></a>
							<a href="Controller?action=manageUsers"><img
								src="buttons/button_manage-users.png" class="midmenu" /></a>
							<a href="Controller?action=manageClasses"><img
								src="buttons/button_manage-classes.png" class="midmenu" /></a>
							<a href="Controller?action=manageSubjects"><img
								src="buttons/button_manage-subjects.png" class="midmenu" /></a>
						</c:when>
						<c:when test="${user.authorization==1 }">
							<a href="Controller?action=editGrades"><img
								src="buttons/button_edit-grades.png" class="midmenu" /></a>
							<a href="Controller?action=announce"><img src="buttons/button_add-news.png"/></a>
							<a href="Controller?action=announcements"><img src="buttons/button_news.png"/></a>
						</c:when>
						<c:when test="${user.authorization==0 }">
							<a href="Controller?action=grades"><img
